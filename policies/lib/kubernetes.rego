...
trusted_namespaces = ["kube-system", "falco", "istio-system", "gatekeeper-system"]
...
privileged_escalation_allowed(c) {
	not has_field(c, "securityContext")
}
...
is_trusted_namespace {
	trusted_namespaces[_] = namespace
}

is_trusted_image(repos) {
	  containers[container]
          image:= container.image
          satisfied := [good | repo = repos[_] ; good = startswith(image, repo)]
          any(satisfied)
}
